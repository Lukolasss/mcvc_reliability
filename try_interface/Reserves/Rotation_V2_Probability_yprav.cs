...
using System.IO;
//СМЕШАННОЕ РЕЗЕРВИРОВАНИЕ С РОТАЦИЕЙ С УЧЕТОМ ВЕРОЯТНОСТИ ПЕРЕКЛЮЧЕНИЯ (для программы управления)
namespace try_interface.Reserves
{
    [Serializable]
    class Rotation_V2_Probability_yprav : Reserve
    {
        ...
        public double TS;//вероятность, требуемая по ТЗ

        private double[] first_fail;//первые отказы

        private new List<List<double>> slegka;
        private List<double[]>[] Periods;//периоды в каждом эксперименте для различного количества оставшихся резервов

        public Rotation_V2_Probability_yprav(int n, int k, int Exp, double lambdaR, double lambdaO, double[] Nagr, double T, double possofswitch, double pos_TS, bool BSV = false) : base(n, k, Exp, lambdaR, lambdaO, Nagr, BSV)
        {
           ...
           Periods = new List<double[]>[numOfExp];
            T_exp = T;
            TS = pos_TS;
 ...
 //вероятность переключения
	...
	//статистика для отладки
        ...
        public List<double[]> Get_Periods(int exp)
        {
            return Periods[exp];
        }
...
//CurrentPeriods-текущие циклы
        ...
        {
            double Treal_Sum = (i > 0) ? (((N + 1 + currentReserves * lOj / lRab) / (N + 1 + currentReserves)) * CurrentPeriods.GetRange(0, i).Sum())
...
//вычисление суммарного времени пройденных полных циклов ротации 
        ...
        {
            double Treal_Sum = (i > 0) ? (CurrentPeriods.GetRange(0, i).Sum())
                : (CurrentPeriods[0]);
            return Treal_Sum;
        }
...
//поиск минимальной наработки
                ...
                else//дошли до Тэ без отказа -> ротация закончилась
                {
//реконфигурация схемы в соответствии со сменой мест каналов во время ротации
                    int[] f = changingPlaces(N, currentReserves, numofperekl + 1);
                    List<double> c = new List<double>(a);
...
//функция расчета наработки
        public override void CalculateTime(Main_Form form1)
        {
            for (int i = 0; i < numOfExp; i++)//цикл по количеству экспериментов
            {
                ...
                List<double> t_o = new List<double>();
               
                while (currentReserves > 0 && !flag_withouRot)//пока есть ротация
                {
//заполнение массива времен отказов
                    if (currentReserves == K)
                    {
                        t_o.Add(0);
                    }
                    else
                    ...
                    //вызов функции расчета периодов

                    double[] per = Calc.GetArr_T_tc_T_p(N, currentReserves + 1, lRab, lOj, 
                        t_o.ToArray(), K - currentReserves, T_exp, TS, POSOFSWITCH, CoefNagr[CoefNagr.Count()-1-(K - currentReserves)], (i<=100));
                    if (Calc.flag_rot)//периоды рассчитаны
                    {
                        per[per.Count() - 1] += T_exp - per.Sum();//корректировка последнего периода
                        Periods[i].Add(per);
                        //вызов функции расчета до отказа
                        Tuple<bool, List<double>> s = PoiskOtkaza(per,
...
//загрузка из файла
        public Rotation_V2_Probability_yprav Load(string filename)
        {
            BinaryFormatter bin = new BinaryFormatter();
            FileStream fs = new FileStream(filename, FileMode.Open, FileAccess.Read);
            Rotation_V2_Probability_yprav res = (Rotation_V2_Probability_yprav)bin.Deserialize(fs);
            fs.Close();
            return res;
        }
//вывод для отчета
        public override string Accumilate_Result(double needed_time, bool changed_num)
        {
            ...
            Result += "Вероятность по ТЗ:";
            Result += Convert.ToString(this.TS) + "\n";
            ...
            Result += "Внутренние переменные из программы управления:\n" + Calc.Stat;
            Result += "Циклы полной ротации:\n";
            for (int e = 0; e < numOfExp && e < 100; e++)
            {
                if (e==1)
                    Result += "во " + Convert.ToString(e + 1) + "-ом эксперименте:";
                else
                Result += "в " + Convert.ToString(e + 1) + "-ом эксперименте:";
               
                    var per = this.Get_Periods(e);
                    foreach (double[] p in per)
                    {
                    for(int t = 0; t < p.Count(); t++)
                        Result += Convert.ToString(p[t]) + " ч.";
                    }
                    Result += "\n";
            }
            Result += "Результаты:\n";
        ...
        public Tuple<double, List<double>> CountPossibility(int currentReserves, double[] b, List<List<double>> posstab, List<List<Tuple<bool, double>>> switchtab, double[] CurrentPeriods, double[,] m)
        {
            ...
            foreach (List<double> list in posstab)
            {
                ...
                {
                    numofsw++;
                    if (poss > POSOFSWITCH && numofsw < per)//0.99 - надо будет поменять на задаваемую переменную
                    {
                        per = numofsw;
                        y = z;
                    }
